...
void recibir_ordenes_kernel(int conexion_filesystem_kernel, char* ruta_bitmap, char* ruta_archivo_bloques){
	while(conexion_filesystem_kernel){
    	...
    	switch(operacion->codigo_operacion){
			case ABRIR_ARCHIVO:
				abrir_o_crear_archivo(nombre_archivo, conexion_filesystem_kernel, ruta_bitmap);
				break;
...
{	
	...
	if(dir != NULL) {
		while ((ent = readdir(dir)) != NULL) {
			...
			crear_estructura_fcb(ruta_fcb, ruta_bitmap);
			free(ruta_fcb);
		}
		closedir(dir);	
	} else {
...
void crear_estructura_fcb(char* ruta, char* ruta_bitmap) //habria que llamarlo crear fcb
{
	...
	setear_bit(fcb->indirect_pointer, bitarray, ruta_bitmap);

	//Actualizo los bloques que usa el fcb en el bitmap
	setear_bit(fcb->direct_pointer, bitarray, ruta_bitmap);
	...
	if (bytes_a_guardar > 0){
		...
		for (int i = 0; i < bloques_necesarios; i++)
		{
			...
			setear_bit(bloque_index, bitarray, ruta_bitmap); //DESHARCODEAR DESPUES
...
void abrir_o_crear_archivo(char *nombre_archivo, int conexion_filesystem_kernel, char* ruta_bitmap)
{
	...
	{
		crear_archivo_fcb(nombre_archivo, ruta_bitmap);
...
void *crear_archivo_fcb(char *nombre_archivo, char* ruta_bitmap)
{
	...
	config_save(fcb);
	crear_estructura_fcb(ruta, ruta_bitmap);
	free(ruta);
	return NULL;
}
