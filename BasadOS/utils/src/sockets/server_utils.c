...
t_paquete *recibir_paquete(int conexion_socket)
{
	...
	if (recv(conexion_socket, &(paquete->codigo_operacion), sizeof(int), MSG_WAITALL) <= 0)
	{
	...
	if (recv(conexion_socket, &(paquete->buffer->size), sizeof(uint32_t), MSG_WAITALL) <= 0)
	{
	...
	if (paquete->buffer->size > 0)
	{
		...
		if (recv(conexion_socket, paquete->buffer->stream, paquete->buffer->size, MSG_WAITALL) <= 0)
		{
